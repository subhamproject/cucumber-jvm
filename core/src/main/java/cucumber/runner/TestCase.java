...
class TestCase implements cucumber.api.TestCase {
    ...
    void run(EventBus bus) {
        ...
        ScenarioImpl scenarioResult = new ScenarioImpl(bus, this, pickle);

        for (HookTestStep before : beforeHooks) {
            Result stepResult = before.run(this, bus, pickle.getLanguage(), scenarioResult, dryRun);
            skipNextStep |= !stepResult.is(Result.Type.PASSED);
            scenarioResult.add(stepResult);
        }

        for (PickleStepTestStep step : testSteps) {
            Result stepResult = step.run(this, bus, pickle.getLanguage(), scenarioResult, skipNextStep);
            skipNextStep |= !stepResult.is(Result.Type.PASSED);
            scenarioResult.add(stepResult);
        }

        for (HookTestStep after : afterHooks) {
            Result stepResult = after.run(this, bus, pickle.getLanguage(), scenarioResult, dryRun);
            scenarioResult.add(stepResult);
        }
